...
class KDLoss(nn.Module):
    ...
    # def forward(self, out_s, out_t, target, alpha, loss_aug):
...
class BKDLoss(nn.Module):
    ...
    # def forward(self, out_s, out_t, output_a, target, alpha, lam2, target2):
    ...
    #                     pred_t,
    #                     reduction='none').mean(dim=0)
    #     loss_a = self.CELoss(output_a, target) * lam2 + self.CELoss(output_a, target2) * (1. - lam2)
    #
    #     kd_loss = kd.sum() * self.T * self.T
    #     # ce_loss = self.CELoss(out_s, target)
    #     loss = alpha * kd_loss + loss_a
    #
    #     return loss, kd
    ...
    #     loss = alpha * kd_loss
    #
    #     return loss, kdawdawdawd
    #
    # def forward(self, out_s, out_t, target, alpha, output1):
    #     pred_t = F.softmax(out_t/self.T, dim=1)
    #     if self.weight is not None:
    #         pred_t = pred_t * self.weight
    #         pred_t = pred_t / pred_t.sum(1)[:, None]
    #     kd = F.kl_div(F.log_softmax(output1/self.T, dim=1),
    #                     pred_t,
    #                     reduction='none').mean(dim=0)
    #     kd_loss = kd.sum() * self.T * self.T
    #     ce_loss = self.CELoss(out_s, target)
    ...
    #                     reduction='none').mean(dim=0)
    #     kd_loss = kd.sum() * self.T * self.T
    #     ce_loss = self.CELoss(output1, target)
    #     loss = alpha * kd_loss + ce_loss
    #
    #     return loss, kd
